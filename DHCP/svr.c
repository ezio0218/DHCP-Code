...
int main(int argc, char *argv[]){
    ...
    int x = 1;
    if(setsockopt(sock, SOL_SOCKET, SO_BROADCAST | SO_REUSEADDR, &x, sizeof(int))==0)
    ...
    // Broadcast
    memset(&broadcastAddr, 0, sizeof(broadcastAddr));/*Set broadcast address zero*/
    broadcastAddr.sin_family = AF_INET; /* Internet addr family IPV4*/
    ...
    if ((bind(sock, (struct sockaddr *) &svrAddr, sizeof(svrAddr))) < 0)
       printf("bind() failed.\n");
    selfAddr.sin_addr.s_addr = inet_addr(broadcastIP);
    while(1)
    {
